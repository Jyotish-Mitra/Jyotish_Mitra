...
exports.handler = async (event) => {
  ...
  try {
    ...
    const [kundli, planetPosition, dashaPeriods, birthDetails, chartSvg, kpData] =
      await Promise.all([
        ...
        callProkerala(
          ...
          // Chart SVG fail ho to bhi baaki data chala jaye
          console.error("Chart SVG fetch failed:", err.message);
          return null;
        }),
        callProkerala("kp-birth-details", commonParams, token).catch((err) => {
          // KP endpoint abhi available nahi ho sakta - fail-safe
          console.error("KP data fetch failed:", err.message);
          return null;
        }),
      ]);
